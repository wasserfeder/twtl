...
def wfse_constructor(task_case):
    ap = set(['A', 'B', 'C', 'D','E','O','B','Q']) # set of atomic propositions
    ...
    if (user_preference == '3'): 
        ...
        in_symbol = wfse.bitmap_of_props(set(['E']))
        out_symbol = wfse.bitmap_of_props(set(['D']))
        # out_symbol = -1
        ...
        # wfse.g.add_edge('q1', 'q12', attr_dict={'symbols': weighted_symbols})
        # wfse.g.add_edge('q4', 'q12', attr_dict={'symbols': weighted_symbols})
        ...
        # wfse.g.add_edge('q7', 'q12', attr_dict={'symbols': weighted_symbols})
...
def main():
    ...
    ## ref: [H^3 !A]^[0, 8] * [H^2 B & [H^4 C]^[3, 9]]^[2, 19]',

    # phi_sdc = '[W !O & !O2 & [H^2 E]^[0,2] * [H^3 D]^[0,5] * [H^1 A] ^ [0,7] * [H^1 B] ^ [0,10]] ^ [0,20]'

    # phi_sdc = ' [H^3 !O & [H^1 A]^[0,10]] ^ [0,12] ' ##extend this

    phi_sdc = 'H^20 !O  & H^7 !Q & ([H^1 E]^ [0,3] * [H^2 D] ^ [0,7] * [H^1 A] ^ [0,5]) '
    # phi_sdc = '(H^15 !O) & [H^1 A] ^ [0,3]'
    ...
    twtl_synthesis(phi_sdc, '../data/sdc_word_complex.yaml', wfse)
